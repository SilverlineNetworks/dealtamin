          ...
          <tr>
            ...
            <td>
                <label class="switch" title="Permission required !!">
                    ...
                    id="isWorking{{$schedules->id}}"
                    data-id="{{$schedules->id}}"
                    data-empid ="{{$schedules->employee_id}}"
                    value="yes"
                    @if($schedules->is_working == 'yes') checked @endif @if(!auth()->user()->roles()->withoutGlobalScopes()->first()->hasPermission('update_employee_schedule')) disabled @endif>
            ...
            <td id="editButton{{$schedules->id}}">
                @if($schedules->is_working == 'yes')
                    ...
                    data-id="{{$schedules->id}}"
                    data-empid ="{{$schedules->employee_id}}"
                    class="btn btn-primary btn-circle edit-details">
...
<script>
    $('body').on('click', '.edit-details', function() {
        ...
        var startInput = '<input type="text" class="form-control startinput" id="starttimeinput-'+id+'" autocomplete="off" value="'+hiddenstarttime+'">';
        $('#startinputId'+id).html(startInput);

        var hiddenendtime = $('#hiddenendtime'+id).val();
        var endInput = '<input type="text" class="form-control endinput" id="endtimeinput-'+id+'" autocomplete="off" value="'+hiddenendtime+'">';
            $('#endinputId'+id).html(endInput);

        var update = '<a href="javascript:;" class="btn btn-success btn-circle update-schedule"  data-id="'+id+'" id="updateButton'+id+'" data-empid ="'+employeeId+'"><i class="fa fa-check"></i></a>';

        var cancel = '<a href="javascript:;" class="btn btn-danger btn-circle cancel-schedule" id="cancelButton'+id+'" data-id="'+id+'" data-empid ="'+employeeId+'"><i class="fa fa-close"></i></a>';
        var space = '&nbsp;';
    ...
    $('body').on('click', '.cancel-schedule', function() {
        ...
        var $input = $('<a href="javascript:;" class="btn btn-primary btn-circle edit-details" data-id="'+id+'" data-empid="'+empid+'" id="editButton'+id+'"><i class="fa fa-pencil" aria-hidden="true"></i></a>').val( $updateButton.text());
