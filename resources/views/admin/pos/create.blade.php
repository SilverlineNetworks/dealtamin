                    ...
                    <div class="card-body">
                        ...
                        <div class="row">
                            <table class="table table-condensed" id="product-table">
                                <thead>
                                    <tr>
                                        <th width="30%">@lang('app.product')</th>
                                        <th width="20%">@lang('app.price')</th>
                                        <th width="23%">@lang('app.quantity')</th>
                                        <th class="text-right">@lang('app.subTotal')</th>
                                        <th><i class="fa fa-gear"></i></th>
                ...
                <div class="card">
                    <div class="card-body">
                        <div class="row pos-calculations">
                            ...
                            <div class="col-md-6">
                                <h4 id="total-cart">{{ $settings->currency->currency_symbol }}0</h4>
...
@push('footer-js')
    ...
    <script>
                        ...
                        success: function (response) {
                            if(response.status != 'fail'){
                                ...
                                var discountElement =     '<div class="col-md-6 border-bottom couponDiscountBox" id="couponDiscountBox">'+
                                                            ...
                                                            '</div>'+
                                                            '<div class="col-md-6 border-bottom couponDiscountBox">'+
                                                            ' <h5 id="coupon-discount-amount">-'+currencySymbol +response.amount+'</h5>'+
                                                            '</div>';
                                ...
                                $('#couponCodeAmonut').html(response.amount);
                                $('#couponCode').html(response.couponData.title);
                                calculateTotal();
                            } else {
                                removeCoupon ();
        ...
        $("body").on('click', '.add-to-cart', function () {
            ...
            if(isAdded === false && serviceId !== undefined){
                ...
                let productRow =  '<tr>\n' +
                    ...
                    '                                <td><input type="hidden" name="product_cart_prices[]" class="product-cart-price-'+productId+'" value="'+productPrice+'">{{ $settings->currency->currency_symbol }}'+productPrice+'</td>\n' +
                    '                                <td><div class="input-group">\n' +
                    '                  <div class="input-group-prepend">\n' +
        ...
        function calculateProductTotal() {
            ...
            $("#product-sub-total").html("{{ $settings->currency->currency_symbol }}"+cartSubTotal.toFixed(2));
            ...
            $("#product-sub-total").html("{{ $settings->currency->currency_symbol }}"+cartTotal);
