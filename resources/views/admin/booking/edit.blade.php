                ...
                <div class="dropdown">
                    ...
                    <div class="dropdown-menu">
                        @foreach($products as $product)
                            ...
                            data-product-id="{{ $product->id }}" href="javascript:;">{{ ucwords($product->name) }}</a>
                        @endforeach
                    </div>
                </div>
            </div>
                ...
                @foreach($booking->items as $key=>$item)
                    ...
                    <tr>
                        ...
                        <td class="text-right cart-subtotal-{{ $item->id }}">
                            {{ $settings->currency->currency_symbol.number_format((float)($item->unit_price  * $item->quantity), 2, '.', '') }}
                        </td>
                        <td>
        ...
        <div class="col-md-6 border-top">
            <div class="row">
                <div class="col-md-12">
                    <table class="table table-condensed">
                        <tr class="h6">
                            <td class="border-top-0 text-right w-50">@lang('app.service') @lang('app.subTotal')</td>
                        ...
                        @endif
                        <tr class="h6">
                            <td class="text-right">@lang('app.service') @lang('app.total')</td>
                            <td id="cart-total">{{ $settings->currency->currency_symbol.number_format((float)($booking->amount_to_pay - $booking->product_amount), 2, '.', '') }}
                                <input type="hidden"  id="cart-total-input">
                            </td>
                        </tr>
                        <tr class="h6">
                            <td class="text-right">@lang('app.product') @lang('app.total')</td>
                            <td id="product-total">{{ $settings->currency->currency_symbol.number_format((float)($booking->product_amount), 2, '.', '') }}
                                <input type="hidden"  id="product-total-input">
                            </td>
                        </tr>
                        <tr class="h5">
                            <td class="text-right">@lang('app.grand') @lang('app.total')</td>
                            <td id="total-cart">{{ $settings->currency->currency_symbol.number_format((float)$booking->amount_to_pay, 2, '.', '') }}
                                <input type="hidden"  id="cart-total-input">
                            </td>
                        </tr>
                    </table>
                    {{-- this input is used, to redirect after payment --}}
                    <input type="hidden" id="current_url" name="current_url" value="booking_url">
                </div>
            </div>
...
<script>
    ...
    function updateProductQuantity(productId, qty) {
        ...
        $('.product-subtotal-'+productId).html("{{ $settings->currency->currency_symbol }}"+subTotal.toFixed(2));
    ...
    function calculateProductTotal() {
        ...
        $("#product-total").html("{{ $settings->currency->currency_symbol }}"+cartSubTotal.toFixed(2));
    ...
    function total() {
        ...
        $("#total-cart").html("{{ $settings->currency->currency_symbol }}"+totalCart);
        $("#payment-modal-total").html("{{ $settings->currency->currency_symbol }}"+totalCart);
    }
    ...
    $('body').on('click', '.add-item', function(){
        ...
        let item = '<tr>\n' +
            ...
            '                        '+serviceName+'</td>\n' +
            '                    <td><input type="hidden" class="cart-price-'+serviceId+'" name="item_prices[]" value="'+servicePrice+'">{{ $settings->currency->currency_symbol }}'+servicePrice+'</td>\n' +
            '                    <td>\n' +
            '                        <div class="input-group">\n' +
            '                            <div class="input-group-prepend">\n' +
    ...
    $('body').on('click', '.add-product', function(){
        ...
        let item = '<tr>\n' +
            ...
            '                        '+productName+'</td>\n' +
            '                    <td><input type="hidden" name="product_prices[]" class="product-price-'+productId+'" value="'+productPrice+'">{{ $settings->currency->currency_symbol }}'+productPrice+'</td>\n' +
            '                    <td>\n' +
            '                        <div class="input-group">\n' +
            '                            <div class="input-group-prepend">\n' +
            ...
            '                        </div>\n' +
            '                    </td>\n' +
            '                    <td class="text-right product-subtotal-'+productId+'">{{ $settings->currency->currency_symbol }}'+productPrice+'</td>\n' +
            '                    <td>\n' +
            '                        <a href="javascript:;" class="btn btn-danger btn-sm btn-circle delete-product-row"><i class="fa fa-times" aria-hidden="true"></i></a>\n' +
            '                    </td>\n' +
            '                </tr>';
