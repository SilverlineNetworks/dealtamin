                ...
                <div class="row">
                    <div class="col-lg-7 col-md-12">
                        <div class="deal_detail_img position-relative">
                            <img src=" {{ $deal->deal_image_url }} " alt="Image" />
                        </div>
                    </div>
                    <div class="col-lg-5 col-md-12 deal_detail_box">
                        <h3 class="mt-lg-1 mt-4">{{ $deal->company->company_name }}</h3>
                        <h2>{{ $deal->title }}</h2>
                        ...
                        <div class="deal_detail_offer_with_price d-flex align-items-center">
                            @if($deal->original_amount - $deal->deal_amount > 0)
                            <i>
                                @if($deal->discount_type=='percentage')
                                    {{$deal->percentage}}%
                                @else
                                {{ $settings->currency->currency_symbol }}{{$deal->original_amount - $deal->deal_amount}}
                                @endif
                                @lang('app.off')
                            </i>
                            @endif
                            <p>{{ $settings->currency->currency_symbol }}{{ $deal->deal_amount }}
                                <span>@if($deal->deal_amount < $deal->original_amount){{ $settings->currency->currency_symbol }}{{ $deal->original_amount }}@endif</span>
                            </p>
                        </div>
