                                    ...
                                    <div class="media coupon_box view_latest_coupon_box">
                                        ...
                                        <div class="media-body coupon_code_box text-center position-relative">
                                            ...
                                            @elseif (is_null($coupon->amount) && !is_null($coupon->percent))
                                                {{$coupon->amount}}%
                                            @else
                                                @lang('app.maxAmountOrPercent', ['percent' => $coupon->percent, 'maxAmount' => $settings->currency->currency_symbol.$coupon->amount])
                                            @endif
            ...
            $.easyAjax({
                ...
                success: function (response) {
                    if(response.coupon_count==0) {
                        ...
                        $('#filtered_coupons').html('<div  class="mt-4 no_result text-center"><img src="'+image_path+'" class="mx-auto d-block" alt="Image" width="40%"/><h2 class="mt-3">@lang("messages.noResultFound") :(</h2><p>@lang("messages.checkSpellingOrUseGeneralTerms")</p></div>');
                    }
                    else {
