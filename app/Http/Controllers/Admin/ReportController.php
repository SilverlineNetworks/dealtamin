    ...
    public function salesTable(Request $request)
    {
        ...
        return \datatables()->of(collect($items))
            ...
            ->editColumn('amount', function ($row) {
                return $this->settings->currency->currency_symbol.$row->totalAmount;
            })
    ...
    public function tabularTable(Request $request)
    {
        ...
        return \datatables()->of(collect($items))
            ...
            ->editColumn('amount', function ($row) {
                ...
                if($status=='completed'){
                    return '<i data-toggle="tooltip" data-original-title="'.__('app.payment')." ".__('app.done').'" class="fa fa-check-square amount-complete"></i> '.$this->settings->currency->currency_symbol.$row->amount_to_pay;
                }else {
                    return ' <i data-toggle="tooltip" data-original-title="'.__('app.payment')." ".__('app.pending').'" class="fa fa-times-circle amount-due"></i> '.$this->settings->currency->currency_symbol.$row->amount_to_pay;
                }
            })
