...
class FrontController extends FrontBaseController
{
    ...
    public function index()
    {
        ...
        if (request()->ajax())
        {
            ...
            ->with(['services' => function ($query) {
                $query->active()->withoutGlobalScope('company');
            }])
        ...
        /* COUPON */
        $coupons = Coupon::active()->withoutGlobalScope('company')->with([
    ...
    public function serviceDetail(Request $request, $categorySlug, $serviceSlug)
    {
        ...
        $sub_services = BusinessService::where('parent_id', $service_id)
        ->withoutGlobalScope('company')
        ->with([
                'company' => function($q){
                    $q->withoutGlobalScope('company');
    ...
    {
        $deal = Deal::withoutGlobalScope('company')->with([
            'company' => function($q){
                $q->withoutGlobalScope('company');
            },
            'location' => function($q){
    ...
    public function AllServices(Request $request)
    {
        ...
        if($request->ajax())
        {
            $services = BusinessService::where('parent_id', 0)->withoutGlobalScope('company')
                ->with([
                    'location' => function($q) { $q->withoutGlobalScope('company'); } ,
    ...
    public function store_company(RegisterCompany $request)
    {
        if(request()->ajax())
        {
            ...
            $user->attachRole(Role::select('id', 'name')->where(['name' => 'administrator', 'company_id' => $company->id])->first()->id);
