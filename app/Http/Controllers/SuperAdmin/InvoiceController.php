        ...
        return Datatables::of($offline)
            ...
            ->addColumn('action', function ($row) {
                if($row->method == 'Stripe' && $row->invoice_id){
                    return '<a href="'.route('superadmin.stripe.invoice-download', $row->invoice_id).'" class="btn btn-primary btn-circle waves-effect" data-toggle="tooltip" data-original-title="Download"><span></span> <i class="fa fa-download"></i></a>';
                }

                if($row->method == 'Paypal'){
                    return '<a href="'.route('superadmin.paypal.invoice-download', $row->id).'" class="btn btn-primary btn-circle waves-effect" data-toggle="tooltip" data-original-title="Download"><span></span> <i class="fa fa-download"></i></a>';
                }

                if($row->method == 'Razorpay'){
                    return '<a href="'.route('superadmin.razorpay.invoice-download', $row->id).'" class="btn btn-primary btn-circle waves-effect" data-toggle="tooltip" data-original-title="Download"><span></span> <i class="fa fa-download"></i></a>';
                }

                if($row->method == 'Offline') {
                    return '<a href="'.route('superadmin.offline.invoice-download', $row->id).'" class="btn btn-primary btn-circle waves-effect" data-toggle="tooltip" data-original-title="Download"><span></span> <i class="fa fa-download"></i></a>';
                }
        ...
        if (\request()->ajax()) {
            return \datatables()->of($notTransferedPayments)
                ...
                ->editColumn('amount', function ($row) {
                    return $row->currency->currency_symbol . ' ' . $row->amount;
                })
                ->editColumn('application_fee', function ($row) {
                    ...
                    return $row->currency->currency_symbol . ' ' . $applicationFee;
                })
                ->editColumn('method', function ($row) {
                    return $row->gateway;
                })
